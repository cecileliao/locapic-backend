...
router.post('/places', (req, res) => {
  ...
  .catch(error => {
    res.json({ result: false, error: 'Failed to create place' });
  });
});
...
router.get('/places/:nickname', (req, res) => {
  ...
  .catch(error => {
    res.json({ result: false, error: 'Failed to fetch places' });
  });
});
...
router.delete('/places', (req, res) => {
  ...
  Place.deleteOne({ nickname: { $regex: new RegExp(nickname, 'i') }, name })
  .then((deletedDoc) => {
  ...
  .catch(error => {
    res.json({ result: false, error: 'Failed to delete place' });
  });
});
